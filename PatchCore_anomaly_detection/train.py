...
class STPM(pl.LightningModule):
    ...
    def save_anomaly_map(self, anomaly_map, input_img, gt_img, file_name, x_type):
        ...
        # #mark anomalies
        # mask = anomaly_map_norm
        # # mask[mask > threshold] = 1
        # # mask[mask <= threshold] = 0
        # kernel = morphology.disk(4)
        # mask = morphology.opening(mask, kernel)
        # mask *= 255
        # vis_img = mark_boundaries(input_img, mask, color=(1, 0, 0), mode='thick')

        # # save marked images
        # cv2.imwrite(os.path.join(self.sample_path, f'{x_type}_{file_name}_mark.jpg'), input_img)
        # cv2.imwrite(os.path.join(self.sample_path, f'{x_type}_{file_name}_amap_mark.jpg'), anomaly_map_norm_hm)
        # cv2.imwrite(os.path.join(self.sample_path, f'{x_type}_{file_name}_amap_on_img_mark.jpg'), hm_on_img)
        # cv2.imwrite(os.path.join(self.sample_path, f'{x_type}_{file_name}_gt.jpg_mark'), gt_img)
    ...
    def test_step(self, batch, batch_idx): # Nearest Neighbour Search
        ...
        # print("HOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO",score_patches)
    ...
    def test_epoch_end(self, outputs):
        ...
        # print("HOOOOOOOOOOOOOOOOOOOOOO",type(self.gt_list_img_lvl[0]),self.gt_list_img_lvl[0])
...
def get_args():
    ...
    parser.add_argument('--dataset_path', default=r'/media/sagan/3ACA231948FF6FA0/pre_arranged_database_viva_forcflow/') # 'D:\Dataset\mvtec_anomaly_detection')#
    parser.add_argument('--category', default='Aside0')
