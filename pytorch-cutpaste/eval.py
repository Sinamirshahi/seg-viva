...
def get_train_embeds(model, size, defect_type, transform, device):
    ...
    test_data = MVTecAT("/media/sagan/3ACA231948FF6FA0/pre_arranged_database_viva_forcflow/", defect_type, size, transform=transform, mode="train")
...
def eval_model(modelname, defect_type, device="cpu", save_plots=False, size=256, show_training_data=True, model=None, train_embed=None, head_layer=8, density=GaussianDensityTorch()):
    ...
    if test_data_eval is None or cached_type != defect_type:
        ...
        test_data_eval = MVTecAT("/media/sagan/3ACA231948FF6FA0/pre_arranged_database_viva_forcflow/", defect_type, size, transform = test_transform, mode="test")
    ...
    if model is None:
        ...
        head_layers = [512]*head_layer+[512]
        print(head_layers)
        weights = torch.load(modelname)
        # print(weights["out.weight"].shape[0])
        # exit(-1)
...
if __name__ == '__main__':
    ...
    args = parser.parse_args()
    print(args)
    all_types = ['Aside0','Aside1','Cside0','Cside1','Dside0','Dside1']
