...
def parse_args():
    ...
    parser.add_argument('--resize', type=int, default=380)
...
def show_sample_images(dataloader, n, class_name):
    ...
    fig.tight_layout()
    pic_save_path = os.path.join(save_path, 'sample_%s' % (class_name))
...
def main():
    ...
    os.makedirs(os.path.join(save_path, 'model_pkl_%s' % name), exist_ok=True)
    ...
    data_p = "/media/sagan/3ACA231948FF6FA0/pre_arranged_database_viva/"

    for k, class_name in enumerate(mvtec.CLASS_NAMES):
        train_dataset = mvtec.MVTecDataset(data_p, class_name=class_name, is_train=True)
        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, pin_memory=True)
        test_dataset = mvtec.MVTecDataset(data_p, class_name=class_name, is_train=False)
        ...
        precision, recall, thresholds = precision_recall_curve(gt_mask.flatten().astype(int), scores.flatten())
        ...
        fpr, tpr, _ = roc_curve(gt_mask.flatten().astype(int), scores.flatten())
        per_pixel_rocauc = roc_auc_score(gt_mask.flatten().astype(int), scores.flatten())
        total_pixel_roc_auc.append(per_pixel_rocauc)
        print('pixel ROCAUC: %.3f' % (per_pixel_rocauc))

        fig_pixel_rocauc.plot(fpr, tpr, label='%s ROCAUC: %.3f' % (class_name, per_pixel_rocauc))
        save_dir = save_path + '/' + f'pictures_efficientnet-{args.arch}'
        os.makedirs(save_dir, exist_ok=True)
        plot_fig(test_imgs, scores, gt_mask_list, threshold, save_dir, class_name)

        # class, image ROCAUC, pixel ROCAUC, inference_time
        with open(save_path + '/' + f'efficientnet-{args.arch}-lst.txt', "a") as f:
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(save_path, '%s_lst_roc_curve.png' % name), dpi=100)
